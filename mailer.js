...
async function enviar({ para, assunto, html }) {
  ...
  if (!t) { console.log(`[Mailer] Não configurado. Pulando: ${assunto}`); return; }
  try {
...
function htmlNovoChamado(c, solicitante) {
  return wrap(`
    ...
    <table style="width:100%;border-collapse:collapse;margin:16px 0">
      ...
      <tr><td style="color:#6b7280;padding:6px 0">Título</td><td>${c.titulo}</td></tr>
...
function htmlConcluido(c) {
  return wrap(`
    <h2 style="color:#15803d">✅ Chamado Concluído — ${c.protocolo}</h2>
    <p><strong>${c.titulo}</strong></p>
    <p>Seu chamado foi concluído. Por favor, avalie o atendimento.</p>
...
function wrap(content) {
  return `<div style="font-family:sans-serif;max-width:600px;margin:auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden">
    ...
    <div style="background:#f8fafc;padding:12px 20px;font-size:12px;color:#9ca3af">Este é um e-mail automático. Não responda.</div>
  </div>`;
}

function corPrio(p) { return {Critica:'#dc2626',Alta:'#ea580c',Media:'#ca8a04',Baixa:'#16a34a'}[p]||'#374151'; }

module.exports = { enviar, htmlNovoChamado, htmlAtualizacao, htmlConcluido };
